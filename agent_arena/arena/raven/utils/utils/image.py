...
from .heightmap import reconstruct_heightmaps, pix_to_xyz
...
def perturb(input_image, pixels, set_theta_zero=False):
    ...
    # print('new image shape', input_image.shape)
    # print('new image type', input_image.dtype)
    # print('new image max', np.max(input_image))
    # print('new image min', np.min(input_image))
    # plt.imshow(input_image)
    # plt.show()
...
def apply_rotations_to_tensor(in_tensor, num_rotations, center=None, reverse=False):
    ...
    tensor = Rearrange('n b h w c -> n b c h w')(tensor)
    t_clone = tensor.clone()
    for idx, theta in enumerate(thetas):
        for b in range(B):
            centre = (tuple(center[b]) if center is not None else None)
            tensor[idx, b, ...] = rotate(
                t_clone[idx, b, ...],
                theta,
                center=centre)
    tensor = Rearrange('n b c h w -> n b h w c')(tensor)
