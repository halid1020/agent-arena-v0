...
import agent.algorithm.dreamerV2.networks as networks
import agent.algorithm.dreamerV2.tools as tools
...
class WorldModel(nn.Module):
  ...
  def __init__(self, step, config):
    ...
    self.heads['rgb'] = networks.ConvDecoder(
  ...
  def preprocess(self, obs):
    obs = obs.copy()
    #print('obs image shape', obs['rgb'].shape)
    obs['rgb'] = torch.Tensor(obs['rgb']) / 255.0 - 0.5
    H, W, C = obs['rgb'].shape[-3:]
    ln = len(obs['rgb'].shape)
    before_shape = obs['rgb'].shape[:ln-3]
    

    obs['rgb'] = obs['rgb'].reshape(-1, H, W, C).permute(0, 3, 1, 2)
    
    ### Resize the image to 64*64
    obs['rgb'] = F.interpolate(
      obs['rgb'].reshape(-1, C, H, W), 
      size=self._config.size, mode='bilinear', align_corners=False)\
    .permute(0, 2, 3, 1)\
    .reshape(*before_shape, self._config.size[0], self._config.size[1], C)

    #print('obs image shape', obs['rgb'].shape)
  
    
    obs['reward'] *= self._config.reward_scale
