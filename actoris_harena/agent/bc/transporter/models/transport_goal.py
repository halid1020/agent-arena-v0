...
class TransportGoal:
    ...
    def __init__(self, in_channels, n_rotations, crop_size, verbose=False):  # pylint: disable=g-doc-args
    ...
    def forward(self, in_img, goal_img, p, apply_softmax=True):  # pylint: disable=g-doc-args
        ...
        goal_data = np.pad(goal_img, self.padding, mode='constant')
    ...
    def test(self, in_img, goal_img, p, q, theta):
        """Test."""
        self.eval_mode()

        with torch.no_grad():
            loss = self.train_block(in_img, goal_img, p, q, theta)
    ...
    def train_block(self, in_img, goal_img, p, q, theta):
    ...
    def save(self, fname, verbose=False):
        ...
        if verbose:
            print(
                f"Saving {bold('transport logits')} model to {bold(logits_name)}")
            print(
                f"Saving {bold('transport kernel')}   model to {bold(kernel_name)}")
            print(
                f"Saving {bold('transport goal')}   model to {bold(goal_name)}")
    ...
    def load(self, fname, verbose):
        ...
        if verbose:
            ...
            print(
                f"Loading {bold('transport logits')} model on {bold(device)} from {bold(logits_name)}")
            print(
                f"Loading {bold('transport kernel')} model on {bold(device)} from {bold(kernel_name)}")
            print(
                f"Loading {bold('transport goal')} model on {bold(device)} from {bold(goal_name)}")
