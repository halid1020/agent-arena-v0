...
from arena.deformable_raven.src import tasks, utils


class Environment():

    def __init__(self, disp=False, hz=240, camera_configs=[]):
        ...
        assets_path = os.environ['DEFORMABLE_RAVEN_ASSETS_DIR']
        #print('asset path: ', assets_path)
    ...
    def reset(self, task, last_info=None, disable_render_load=True):
        ...
        id_plane = p.loadURDF('plane/plane.urdf', [0, 0, -0.001])
        id_ws = p.loadURDF('ur5/workspace.urdf', [0.5, 0, 0])

        # Load UR5 robot arm equipped with task-specific end effector.
        self.ur5 = p.loadURDF(f'ur5/ur5-{self.task.ee}.urdf')
        self.ee_tip_link = 12
    ...
    def step(self, act=None):
        ...
        if act and act['primitive']:
            success = self.primitives[act['primitive']](
                act['pose0'],
                act['pose1'])
    ...
    def movej(self, targj, speed=0.01, t_lim=20):
        ...
        while (time.time() - t0) < t_lim:
            ...
            p.stepSimulation()
            if self.task.video_recorder:
                self.task.video_recorder.record_frame()
