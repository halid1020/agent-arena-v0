...
from arena.deformable_raven.src.tasks import Task
from arena.deformable_raven.src import utils as U

BAGS_TO_FILES = {
    1: 'bags/bl_sphere_bag_rad_1.0_zthresh_0.1_numV_257.obj',
    2: 'bags/bl_sphere_bag_rad_1.0_zthresh_0.3_numV_289.obj',
    3: 'bags/bl_sphere_bag_rad_1.0_zthresh_0.4_numV_321.obj',
    4: 'bags/bl_sphere_bag_rad_1.0_zthresh_0.6_numV_353.obj',
    5: 'bags/bl_sphere_bag_rad_1.0_zthresh_0.8_numV_385.obj',
}
...
class BagEnv(Task):
    ...
    def add_zone(self, env):
        ...
        zone_template = 'zone/zone-template.urdf'
        ...
        # For tracking IDs and consistency with existing arena.deformable_raven.src code.
    ...
    def understand_bag_top_ring(self, env, base_pos):
        ...
        #print('f bag', self._f_bag)
        self._top_ring_f = os.path.join(os.environ['DEFORMABLE_RAVEN_ASSETS_DIR'], self._top_ring_f)
...
class BagItemsEasy(BagEnv):
    ...
    def add_cube(self, env, pose, globalScaling=1.0):
        """Andy's arena.deformable_raven.src/block's default size should be (0.04, 0.04, 0.04)."""
        cube_id = p.loadURDF(
                fileName='block/block_for_anchors.urdf',
                basePosition=pose[0],
