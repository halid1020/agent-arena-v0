...
from actoris_harena.agent.utilities.utils import preprocess_observation

class ClothHDF5Dataset(Dataset):
    def __init__(self, 
        data_dir='',  
        ...
        with h5py.File(self.data_dir, 'r') as f:
            ...
            self.N = len(f['trajectory_type'])

            ### If Pick and Place
            if not return_control:
                min_last_step_action_horizons = 1 + min(f['pick_and_place_action_step'][:, -1])
    ...
    def _load(self, episode_id):
        ...
        with h5py.File(self.data_dir, 'r') as f:
            ...
            if not self.return_control:
                ...
                # res_data = {
                #     'action': f['pick_and_place_action'][episode_id][idx[:self.action_horizon]],
                #     'reward': f['reward'][episode_id][idx[:self.action_horizon]],
                #     'rgb': f['rgb'][episode_id][idx[:self.action_horizon+1]].transpose(0, 3, 1, 2)
                # }
            else:
