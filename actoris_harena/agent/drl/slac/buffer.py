...
class ReplayBuffer:
    ...
    def __init__(self, buffer_size, num_sequences, state_shape, action_shape, device, reward_scale):
        self._n = 0
        self._p = 0
    ...
    def sample_latent(self, batch_size):
        ...
        """
        idxes = np.random.randint(low=self.num_sequences + 1, high=self._n, size=batch_size)
        state_ = np.empty((batch_size, self.num_sequences + 1, *self.state_shape), dtype=np.uint8)
        for i, idx in enumerate(idxes):
            #print('self.state_[idx]: ', self.state_[idx].shape)
            # print('self._n', self._n)
            # print('index: ', idx)
            state_[i, ...] = self.state_[idx]
        state_ = torch.tensor(state_, dtype=torch.uint8, device=self.device).float().div_(255.0) - 0.5 #halid modified
        return state_, self.action_[idxes], self.reward_[idxes]*self.reward_scale, self.done_[idxes]
    ...
    def sample_sac(self, batch_size):
        ...
        for i, idx in enumerate(idxes):
            state_[i, ...] = self.state_[idx]
        state_ = torch.tensor(state_, dtype=torch.uint8, device=self.device).float().div_(255.0) - 0.5 #halid modified
        return state_, self.action_[idxes], self.reward_[idxes, -1]*self.reward_scale, self.done_[idxes, -1]
