...
class DeepMindLabyrinth(object):
  ...
  def reset(self, episode_config=None):
    self._done = False
    self._env.reset(episode_config=episode_config)
    obs = self._get_obs()
    return obs
...
class DeepMindControl:
  ...
  def reset(self, episode_config=None):
    time_step = self._env.reset(episode_config)
    obs = dict(time_step.observation)
    obs['image'] = self.render()
    return obs
...
class Atari:
  ...
  def reset(self, episode_config=None):
    with self.LOCK:
      image = self._env.reset(episode_config)
    if self._grayscale:
      image = image[..., None]
...
class CollectDataset:
  ...
  def step(self, action):
    external_state = self._env.step(action)

    #obs, reward, done, info = self._env.step(action)
    obs = external_state['observation']
    ...
    transition['discount'] = external_state.get('discount', np.array(1 - float(done)))
  ...
  def reset(self, episode_config=None):
    external_state = self._env.reset(episode_config)
    transition = external_state['observation'].copy()
...
class TimeLimit:
  ...
  def reset(self, episode_config=None):
    self._step = 0
    return self._env.reset(episode_config)
...
class RewardObs:
  ...
  def step(self, action):
    external_state = self._env.step(action)
    external_state['observation']['reward'] = external_state['reward']
    return external_state

  def reset(self, episode_config=None):
    external_state = self._env.reset(episode_config=episode_config)
    external_state['observation']['reward'] = 0.0
    return external_state
