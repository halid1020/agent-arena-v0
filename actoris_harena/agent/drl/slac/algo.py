...
from agent.algorithm.slac.network import GaussianPolicy, LatentModel, TwinnedQNetwork
from agent.algorithm.slac.utils import create_feature_actions, grad_false, soft_update
import cv2
...
class SlacAlgorithm:
    ...
    def __init__(
        ...
        # Replay buffer.
        self.buffer = ReplayBuffer(buffer_size, num_sequences, state_shape, action_shape, device, reward_scale)
    ...
    def preprocess(self, ob):
        state = torch.tensor(ob.state, dtype=torch.uint8, device=self.device).float().div_(255.0) - 0.5 # halid: modified
        with torch.no_grad():
    ...
    def step(self, env, ob, t, is_random):
        ...
        if is_random:
            action = env.sample_random_action() ## halid: modified
        else:
        ...
        obs = cv2.resize(obs, (64, 64)).transpose(2, 0, 1)
        mask = False if t == env.horizon else done # halid: modified
        
        ob.append(obs, action)
        self.buffer.append(action, reward, mask, obs, done)

        if done:
            t = 0
            external_state = env.reset()
            obs = external_state['observation']['image'] ## halid: modified
            obs = cv2.resize(obs, (64, 64)).transpose(2, 0, 1)
            ob.reset_episode(obs) ## halid: modified
            self.buffer.reset_episode(obs) ## halid: modified
    ...
    def load_model(self, load_dir):
        ...
        self.critic_target.load_state_dict(torch.load(os.path.join(load_dir, "critic.pth")))